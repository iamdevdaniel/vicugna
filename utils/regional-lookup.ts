...
import { Form11Shearing } from "@definitions/types";

export const getRegionalName = (form: Form11Shearing): string => {
  return regionales[form.departamento as keyof typeof regionales]
    .regionales.find(r => r.id === form.asociacionRegional)?.nombre || 'NA';
};

export const getCommunityName = (form: Form11Shearing): string => {
  const departamento = regionales[form.departamento as keyof typeof regionales];
  ...
  const comunidad = regional.comunidades.find(c => c.id === form.comunidadManejadora);
