...
export const validationSchemaForm10 = object().shape({
    ...
    'text-fiber-length': string()
        .matches(/^[0-9]*\.?[0-9]{0,3}$/, 'Este campo debe ser un número con hasta 3 decimales')
        .test('minLength', `La longitud debe ser mayor que ${range.length.min} cm`, isGreaterThanMin(range.length.min))
        .test('maxLength', `La longitud debe ser menor que ${range.length.max} cm`, isLessThanMax(range.length.max))
        .required('Este campo es requerido'),
    'text-live-weight': string()
        .matches(/^[0-9]*\.?[0-9]{0,3}$/, 'Este campo debe ser un número con hasta 3 decimales')
        .test('minWeight', `El peso debe ser mayor que ${range.weight.min} kg`, isGreaterThanMin(range.weight.min))
        .test('maxWeight', `El peso debe ser menor que ${range.weight.max} kg`, isLessThanMax(range.weight.max))
        .required('Este campo es requerido'),
})
